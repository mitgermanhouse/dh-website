...
class Command(BaseCommand):
    def scrape_recipe(self, link):
        ...
        # print(directions)
        return {
            "title": self.replace_space(title),
    ...
    def handle(self, *args, **options):
        ...
        pattern = re.compile(r"[\/]essen[\/]recipes[\/][0-9]+[\/][0-9]+")
        ...
        for a_tag in a_tags:
            ...
            if link_text is not None:
                if pattern.search(link_text) is not None:
    ...
    def add_recipe(self, d):
        r = Recipe(
            recipe_name=d["title"].encode("utf-8"),
