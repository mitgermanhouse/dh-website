...
def combine_ingredients(meals: List[Meal]) -> List[CombinedIngredients]:
	...
	for meal in meals:
		...
		for recipe in meal.recipes.all():
			...
			recipe.associated_meal = meal  # Set associated_meal variable for future reference in the template

			for ingredient in recipe.ingredient_set.all():
				# INFO: If the same recipe gets used multiple times, the same recipe object gets reused multiple times.
				#       This is why in this case we must create a copy of the ingredient and change the associated recipe.
				if not ingredient.recipe is recipe:
					ingredient = copy.copy(ingredient)
					ingredient.recipe = recipe

					# Prevent saving or deleting the copy. This probably isn't necessary...
