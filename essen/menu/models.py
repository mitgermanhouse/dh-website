...
class MealDayTime(models.Model):
    weekday = models.SmallIntegerField(
        choices=[(tag.value, tag.description) for tag in Weekday]
    )
    meal_time = models.CharField(
        max_length=5, choices=[(tag.value, tag.description) for tag in MealTime]
    )

    def __str__(self):
        return " ".join(
            [
                Weekday(self.weekday).description,
                MealTime(self.meal_time).description,
            ]
        )
...
class Meal(models.Model):
    ...
    manual_lateplates = models.ManyToManyField(
        Member, related_name="manual_lateplate_meals", blank=True
    )
    deleted_auto_lateplates = models.ManyToManyField(
        Member, related_name="deleted_auto_lateplate_meals", blank=True
    )
    ...
    def lateplates(self):
        ...
        return (
            (alp_qs.exclude(pk__in=dalp_qs) | mlp_qs)
