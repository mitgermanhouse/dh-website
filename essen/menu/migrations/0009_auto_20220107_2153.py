...
from menu.models import Weekday, MealTime
...
def assign_meal_day_time(apps, schema_editor):
    ...
    for meal in Meal.objects.using(live_layer_db).all():
        if meal.meal_name == 'Sunday Brunch':
            meal.meal_day_time = MealDayTime.objects.filter(weekday=Weekday.SUN.value, meal_time=MealTime.BRUNCH.value).first()
        elif meal.meal_name == 'Sunday Dinner':
            meal.meal_day_time = MealDayTime.objects.filter(weekday=Weekday.SUN.value, meal_time=MealTime.DINNER.value).first()
        elif meal.meal_name == 'Monday Dinner':
            meal.meal_day_time = MealDayTime.objects.filter(weekday=Weekday.MON.value, meal_time=MealTime.DINNER.value).first()
        elif meal.meal_name == 'Tuesday Dinner':
            meal.meal_day_time = MealDayTime.objects.filter(weekday=Weekday.TUE.value, meal_time=MealTime.DINNER.value).first()
        elif meal.meal_name == 'Wednesday Dinner':
            meal.meal_day_time = MealDayTime.objects.filter(weekday=Weekday.WED.value, meal_time=MealTime.DINNER.value).first()
        elif meal.meal_name == 'Thursday Dinner':
            meal.meal_day_time = MealDayTime.objects.filter(weekday=Weekday.THU.value, meal_time=MealTime.DINNER.value).first()
        else:
...
def assign_meal_rating_user(apps, schema_editor):
    ...
    for rating in MealRating.objects.using(live_layer_db).all():
        ...
        if qs.count() > 1:
            raise ValueError(f'Invalid queryset count {qs} for username {rating.username}')
